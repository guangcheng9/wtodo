...
function StorageSection () {
  ...
  useEffect(() => {
    ...
    desktop.storage
      .getPath()
      .then(p => setInfo({ dir: p.dir, defaultDir: p.defaultDir }))
  }, [desktop])
        ...
        <div>
          ...
          <div className='grid grid-cols-3 gap-2'>
            {WALLPAPERS.map(wp => {
              ...
              // Convert "/wallpapers/..." → "wallpapers/..." under file://
              // so thumbnails load in the packaged Electron build too.
              const thumbUrl = isImage ? resolveWallpaperUrl(wp.url) : wp.url
              return (
                <button
                  key={wp.id}
                  onClick={() =>
        ...
        {isDesktop && (
          <>
            ...
            <div>
                ...
                {(
                  [
                    ...
                    },
                    {
                      v: 'desktop',
                      label: '贴近桌面',
                      desc: '隐藏任务栏图标 · 不抢占焦点 · 建议搭配下方贴边自动隐藏使用'
                    }
                  ] as const
                ).map(opt => {
              ...
              {/* <div className="mt-3 flex items-start justify-between gap-2 rounded-md border border-border px-2.5 py-2">
                <Label
                  htmlFor="visible-all-workspaces"
                  className="text-sm font-normal cursor-pointer flex-1"
                >
                  显示在所有虚拟桌面
                  <span className="block text-xs text-muted-foreground mt-0.5">
                    {state.visibleOnAllWorkspaces
                      ? "切换虚拟桌面时小组件会跟随显示"
                      : "仅在当前虚拟桌面显示，切换桌面后隐藏"}
                  </span>
                </Label>
                <Switch
                  id="visible-all-workspaces"
                  checked={state.visibleOnAllWorkspaces}
                  onCheckedChange={(v) =>
                    onChange({ visibleOnAllWorkspaces: v })
                  }
                  className="mt-0.5"
                />
              </div> */}
